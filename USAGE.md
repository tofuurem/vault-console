...
Предпочтение хранится в `localStorage` браузера под отдельным ключом. Это
единственные данные Vault Console в `localStorage`: token остаётся в
`sessionStorage` текущей вкладки, а пароли, secret values, ответы Vault и пути
секретов туда не записываются. Если браузер блокирует `localStorage`, тема
продолжит работать в текущей вкладке, но не сохранится после её закрытия.
...
выполнения и `Esc` для закрытия. После закрытия фокус возвращается на вызвавший
элемент. Недоступные по известным capabilities разделы не добавляются, а
локально недоступные варианты показывают причину; Vault остаётся финальным
источником проверки прав при выполнении операции.
