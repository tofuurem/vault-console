    ...
    <section aria-labelledby="account-heading" className="mx-auto w-full max-w-3xl px-6 py-7">
      ...
      <div className="overflow-hidden rounded-lg border border-background-300 bg-background-50">
        <div className="flex items-center justify-between border-b border-background-200 bg-background-100/60 px-4 py-3">
          ...
          <span className="rounded border border-success-200 bg-success-50 px-1.5 py-0.5 text-[10px] font-semibold text-success-700">
            Enabled
          </span>
        </div>
        ...
        <div className="space-y-5 p-4 sm:p-5">
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              />
              <p id={validation.username ? 'create-user-username-error' : 'create-user-username-help'} className={`mt-1 text-[11px] ${validation.username ? 'text-danger-600' : 'text-foreground-400'}`}>
          ...
          <div>
            <label htmlFor="create-user-mount" className="mb-1.5 block text-xs font-medium text-foreground-700">
              Userpass mount <span className="text-danger-500">*</span>
            </label>
            ...
            {userpassMounts.length === 0 && <p className="mt-1 text-[11px] text-warning-700">No enabled userpass auth mount is visible to this token.</p>}
            {validation.userpassMount && <p className="mt-1 text-[11px] text-danger-600">{validation.userpassMount}</p>}
          </div>
          ...
          <div className="border-t border-background-200 pt-5">
            <div className="mb-2 flex items-end justify-between gap-4">
              ...
              <span className="text-[10px] font-semibold uppercase tracking-wider text-success-700">{strength.label}</span>
            </div>
            ...
            {validation.password && <p className="mt-1 text-[11px] text-danger-600">{validation.password}</p>}
            <div className="mt-2 flex items-center gap-2">
              <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-background-200" aria-hidden="true">
                <div className="h-full rounded-full bg-success-500 transition-all" style={{ width: `${(strength.score / 6) * 100}%` }} />
              </div>
