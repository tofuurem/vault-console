...
export default function BulkSoftDeleteDialog({
  ...
  return (
    <Modal open={open} onClose={submitting ? () => {} : onClose} title="Soft-delete current versions" width="lg">
      <div className="space-y-4 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm leading-5 text-foreground-700">Vault Console verifies each current version before changing it.</p>
            <p className="mt-1 font-mono text-xs text-foreground-500">{mount}/ · {requestedCount} selected secrets</p>
          </div>
        </div>
        {preparing && <SoftDeleteLoading />}
        {error && <SoftDeleteError error={error} onRetry={onRetry} />}
        {preflight && <SoftDeletePreflight preflight={preflight} />}
...
function SoftDeleteLoading() {
  return (
    <div aria-label="Checking selected secrets" className="space-y-2">
      <div className="h-9 animate-pulse rounded-md bg-background-100" />
      <div className="h-9 animate-pulse rounded-md bg-background-100" />
      <p className="text-xs text-foreground-500">Checking exact capabilities and current metadata…</p>
    </div>
  );
}

function SoftDeleteError({ error, onRetry }: { readonly error: string; readonly onRetry: () => void }) {
  return (
...
function SoftDeletePreflight({ preflight }: { readonly preflight: BulkSoftDeletePreflight }) {
  const excluded = summarizeBulkOutcomes(preflight.excluded);
  const undoableCount = preflight.eligible.filter((candidate) => candidate.canUndo).length;
  const omittedEligible = Math.max(0, preflight.eligible.length - MAX_PREVIEW_ITEMS);
  const omittedExcluded = Math.max(0, preflight.excluded.length - MAX_PREVIEW_ITEMS);
  return (
    <>
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        <Summary label="Denied" value={excluded.denied} tone="warning" />
        <Summary label="Unavailable" value={excluded.missing + excluded.failed} tone="neutral" />
      </div>
      {preflight.eligible.length > 0 && (
        <section aria-labelledby="bulk-soft-delete-ready">
          <h3 id="bulk-soft-delete-ready" className="text-[10px] font-semibold uppercase tracking-[0.12em] text-foreground-500">Current versions to soft-delete</h3>
          <ul className="mt-1 max-h-44 divide-y divide-background-200 overflow-y-auto rounded-md border border-background-200">
            {preflight.eligible.slice(0, MAX_PREVIEW_ITEMS).map((candidate) => (
              <li key={candidate.path} className="flex min-h-10 items-center gap-3 px-3 py-2">
                <span className="min-w-0 flex-1 truncate font-mono text-xs text-foreground-800">{candidate.path}</span>
                <span className="text-[10px] text-foreground-500">v{candidate.version}</span>
                <span className={`rounded px-1.5 py-0.5 text-[9px] font-semibold ${
                  candidate.canUndo ? 'bg-success-100 text-success-700' : 'bg-warning-100 text-warning-800'
                }`}>
                  {candidate.canUndo ? 'Undo available' : 'No Undo permission'}
                </span>
              </li>
            ))}
            {omittedEligible > 0 && <li className="px-3 py-2 text-[10px] text-foreground-500">+{omittedEligible} more ready secrets</li>}
          </ul>
        </section>
      )}
      {preflight.excluded.length > 0 && (
        <section aria-labelledby="bulk-soft-delete-excluded">
          <h3 id="bulk-soft-delete-excluded" className="text-[10px] font-semibold uppercase tracking-[0.12em] text-foreground-500">Excluded from this operation</h3>
          <ul className="mt-1 max-h-36 divide-y divide-background-200 overflow-y-auto rounded-md border border-background-200 bg-background-100/40">
            {preflight.excluded.slice(0, MAX_PREVIEW_ITEMS).map((outcome) => (
              <li key={outcome.path} className="flex min-h-9 items-center gap-3 px-3 py-2">
                <span className="min-w-0 flex-1 truncate font-mono text-xs text-foreground-700">{outcome.path}</span>
                <span className="text-[10px] font-semibold uppercase text-foreground-500">{outcome.status}</span>
              </li>
            ))}
            {omittedExcluded > 0 && <li className="px-3 py-2 text-[10px] text-foreground-500">+{omittedExcluded} more excluded secrets</li>}
          </ul>
        </section>
      )}
      <div className="rounded-md border border-warning-200 bg-warning-50 px-3 py-2 text-[11px] leading-5 text-warning-800">
        This is reversible only for rows marked Undo available. Each request targets the current version shown above.
      </div>
    </>
  );
}
