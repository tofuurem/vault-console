...
type SelectedVersions = Readonly<Record<string, readonly number[]>>;
...
export default function BulkDestroyDialog({
  ...
  const [selected, setSelected] = useState<SelectedVersions>({});
  ...
  const toggleVersion = (path: string, version: number) => {
    ...
    setPathVersions(path, current.includes(version)
      ? current.filter((candidate) => candidate !== version)
      : [...current, version]);
  };

  return (
    <Modal open={open} onClose={submitting ? () => {} : onClose} title="Permanently destroy versions" width="lg">
      <div className="space-y-4 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-danger-700">Destroyed secret data cannot be recovered.</p>
            <p className="mt-1 font-mono text-xs text-foreground-500">{mount}/ · {requestedCount} selected secrets</p>
          </div>
        </div>
        {preparing && <DestroyLoading />}
        {error && <DestroyError error={error} onRetry={onRetry} />}
        {preflight && (
          <DestroyPreflight
            preflight={preflight}
            selected={selected}
            selectedVersionCount={selectedVersionCount}
            setPathVersions={setPathVersions}
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button
            ...
            variant="danger"
            loading={submitting}
            disabled={!preflight || selectedVersionCount === 0 || typedMount.trim() !== mount}
...
function DestroyLoading() {
  return (
    <div aria-label="Checking destroy permissions" className="space-y-2">
      ...
      <div className="h-12 animate-pulse rounded-md bg-background-100" />
      <p className="text-xs text-foreground-500">Loading exact version histories and destroy capabilities…</p>
    </div>
  );
}

function DestroyError({ error, onRetry }: { readonly error: string; readonly onRetry: () => void }) {
  return (
...
interface DestroyPreflightProps {
  readonly preflight: BulkDestroyPreflight;
  readonly selected: SelectedVersions;
  readonly selectedVersionCount: number;
  readonly setPathVersions: (path: string, versions: readonly number[]) => void;
  readonly toggleVersion: (path: string, version: number) => void;
}

function DestroyPreflight({
  preflight,
  selected,
  selectedVersionCount,
  setPathVersions,
  toggleVersion,
}: DestroyPreflightProps) {
  const excluded = summarizeBulkOutcomes(preflight.excluded);
  return (
    <>
      <div className="flex flex-wrap gap-2 text-[10px] font-semibold">
        <span className="rounded bg-background-100 px-2 py-1 text-foreground-700">{preflight.eligible.length} paths available</span>
        <span className="rounded bg-danger-100 px-2 py-1 text-danger-700">{selectedVersionCount} versions selected</span>
        {preflight.excluded.length > 0 && (
          <span className="rounded bg-warning-100 px-2 py-1 text-warning-800">
            {excluded.denied} denied · {excluded.missing} missing · {excluded.failed} failed
          </span>
        )}
      </div>
        ...
        <div className="max-h-[42vh] space-y-2 overflow-y-auto pr-1">
          {preflight.eligible.map((candidate) => (
            <DestroyCandidate
              key={candidate.path}
              candidate={candidate}
              selectedVersions={selected[candidate.path] ?? []}
              setPathVersions={setPathVersions}
              toggleVersion={toggleVersion}
            />
          ))}
      ...
      {preflight.excluded.length > 0 && (
        <details className="rounded-md border border-background-200 px-3 py-2 text-xs">
          <summary className="cursor-pointer font-semibold text-foreground-700">Show {preflight.excluded.length} excluded paths</summary>
          <ul className="mt-2 space-y-1">
            {preflight.excluded.map((outcome) => (
              <li key={outcome.path} className="flex gap-2">
                <span className="min-w-0 flex-1 truncate font-mono text-foreground-700">{outcome.path}</span>
      ...
      <div className="rounded-md border border-danger-200 bg-danger-50 px-3 py-2 text-[11px] leading-5 text-danger-700">
        No version is selected automatically. Review every checked version, then type the mount name to confirm this permanent operation.
      </div>
    </>
  );
}
...
function DestroyCandidate({
  ...
  const allSelected = selectedVersions.length === candidate.versions.length;
  return (
    <fieldset className="rounded-md border border-background-200">
      <legend className="sr-only">Versions for {candidate.path}</legend>
      <div className="flex min-h-10 items-center gap-2 border-b border-background-200 bg-background-100/60 px-3 py-2">
        <span className="min-w-0 flex-1 truncate font-mono text-xs font-semibold text-foreground-800">{candidate.path}</span>
        <button
          type="button"
          onClick={() => setPathVersions(
            candidate.path,
            allSelected ? [] : candidate.versions.map((version) => version.version),
          )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2">
        {candidate.versions.map((version) => (
          <label key={version.version} className="flex min-h-11 cursor-pointer items-center gap-2 border-b border-background-100 px-3 py-2 last:border-b-0 sm:[&:nth-last-child(2)]:border-b-0">
            <input
              type="checkbox"
              checked={selectedVersions.includes(version.version)}
              onChange={() => toggleVersion(candidate.path, version.version)}
              aria-label={`Destroy ${candidate.path} version ${version.version}`}
              className="h-4 w-4 accent-danger-600"
            />
            <span className="font-mono text-xs font-semibold text-foreground-800">v{version.version}</span>
            {version.deletionTime && <span className="rounded bg-warning-100 px-1.5 py-0.5 text-[9px] font-semibold text-warning-800">soft-deleted</span>}
          </label>
        ))}
      </div>
    </fieldset>
