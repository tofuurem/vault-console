...
function DiffValue({ value }: { value: unknown }) {
  ...
  return (
    <div className="group flex items-start gap-1">
      ...
      <button type="button" aria-label={revealed ? 'Hide comparison value' : 'Reveal comparison value'} onClick={() => setRevealed((current) => !current)} className="flex h-11 w-11 shrink-0 items-center justify-center rounded text-foreground-400 opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-400 sm:h-5 sm:w-5 sm:opacity-0 sm:group-hover:opacity-100 sm:focus:opacity-100"><i className={`${revealed ? 'ri-eye-off-line' : 'ri-eye-line'} text-[10px]`} aria-hidden="true" /></button>
    </div>
  );
}
        ...
        {readableVersions.length < 2 ? (
          ...
          <>
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="text-[11px] font-medium text-foreground-500">Version A<select aria-label="Version A" value={leftVersion ?? ''} onChange={(event) => setLeftVersion(Number(event.target.value))} className="mt-1 h-11 w-full rounded-md border border-background-300 bg-background-50 px-2 font-mono text-xs text-foreground-900 sm:h-8">{readableVersions.map((version) => <option key={version.version} value={version.version}>v{version.version}</option>)}</select></label>
              <label className="text-[11px] font-medium text-foreground-500">Version B<select aria-label="Version B" value={rightVersion ?? ''} onChange={(event) => setRightVersion(Number(event.target.value))} className="mt-1 h-11 w-full rounded-md border border-background-300 bg-background-50 px-2 font-mono text-xs text-foreground-900 sm:h-8">{readableVersions.map((version) => <option key={version.version} value={version.version}>v{version.version}</option>)}</select></label>
            </div>
            {loading ? <div className="h-28 animate-pulse rounded-md bg-background-100" aria-label="Loading versions" /> : left && right && (
              <div className="overflow-x-auto rounded-md border border-background-200">
